...
####- model selection -####
models <- 1:4 # only indepdent model
...
S_matrix_m1 <- long_format(data[[1]]$median_S, data[[1]]$lower_S, data[[1]]$upper_S, "Independent model")
S_matrix_m2 <- long_format(data[[2]]$median_S, data[[2]]$lower_S, data[[2]]$upper_S, "Independent model + age risk")
S_matrix_m3 <- long_format(data[[3]]$median_S, data[[3]]$lower_S, data[[3]]$upper_S, "Independent model + rho")
S_matrix_m4 <- long_format(data[[4]]$median_S, data[[4]]$lower_S, data[[4]]$upper_S, "Independent model + age risk + rho")
S_data <- bind_rows(S_matrix_m1, S_matrix_m2, S_matrix_m3, S_matrix_m4) %>% 
  mutate(model = factor(model, levels = c("Independent model", "Independent model + age risk",
                                          "Independent model + rho", "Independent model + age risk + rho")))

I_matrix_m1 <- long_format(data[[1]]$median_I, data[[1]]$lower_I, data[[1]]$upper_I, "Independent model")
I_matrix_m2 <- long_format(data[[2]]$median_I, data[[2]]$lower_I, data[[2]]$upper_I, "Independent model + age risk")
I_matrix_m3 <- long_format(data[[3]]$median_I, data[[3]]$lower_I, data[[3]]$upper_I, "Independent model + rho")
I_matrix_m4 <- long_format(data[[4]]$median_I, data[[4]]$lower_I, data[[4]]$upper_I, "Independent model + age risk + rho")
...
####- Fig4A: Age-specific proportion susceptible matrix -####
...
####- Fig4B: plotting 4: annual incidence -####
ggplot(I_data %>% filter(year %in% 1966:1973) %>% 
  ...
  scale_fill_manual(values = brewer.pal(6, "Set3")[c(1,3:5)],
                    labels = c("Time-independent FOI model",  "Plus age-dedpendent FOI", 
...
####- Fig4C : annual incidence by age group in specific year -####
...
I_data1$label[I_data1$model == "Independent model" & I_data1$year == 1968] <- " "
...
ggplot(infect_age, aes(x = factor(year), y = meanage, 
  ...
  scale_color_manual(values = brewer.pal(9, "Set1")[c(1,3,4,5)],
                     label = c("Time-independent FOI model",  "Plus age-dedpendent FOI", 
                               "Plus seroreversion", "Plus age-dedpendent FOI and seroreversion")) +
...
# tiff("../result/fig4.tiff", width = 40, height = 42, units = "cm", compression = "lzw", res = 300)
# 
